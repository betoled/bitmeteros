...
void sendEmail(struct Alert* alert);
...
void processAlertStatus(SOCKET fd, struct NameValuePair* params){
    ...
    while (alert != NULL) {
		...
		if (total >= alert->amount && alert->hasnotified == 0 && alert->email != NULL && alert->smtpserver != NULL) {
...
void sendEmail(struct Alert* alert){
  ...
  if(curl) {
    ...
    curl_easy_setopt(curl, CURLOPT_USERNAME, alert->smtpuser);
    curl_easy_setopt(curl, CURLOPT_PASSWORD, alert->smtppassword);

    /* This is the URL for your mailserver. Note the use of smtps:// rather
     * than smtp:// to request a SSL based connection. */
    curl_easy_setopt(curl, CURLOPT_URL, strcat("smtps://", alert->smtpserver));
...
/*#ifdef SKIP_HOSTNAME_VERIFICATION
     ...
     * details.
     */
    curl_easy_setopt(curl, CURLOPT_MAIL_FROM, alert->smtpuser);

    /* Add two recipients, in this particular case they correspond to the
     * To: and Cc: addressees in the header, but they could be any kind of
     * recipient. */
    recipients = curl_slist_append(recipients, alert->email);
    recipients = curl_slist_append(recipients, alert->smtpuser);
